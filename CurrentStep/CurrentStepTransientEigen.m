...
for ii = 1 : length(No_N)
    ...
    plot(h_a, N_vec1(ind_inacc), Lambda{ii}(ind_inacc),'-rx','MarkerIndices',(length(Lambda{ii}(ind_inacc)):-8:1),'Markersize',10);
end

plot(h_a, N_vec1((1:N_acc(end))),Lambda{end}(1:N_acc(end)),'-b.','MarkerIndices',1:8:N_acc(end),'Markersize',15);
...
h_l = plot(h_a, N_vec1 ,Lambda_lin,'--','Color',[1,1,1]*0.1);
...
for ii = 1 : length(No_N)
    ...
    plot(h_a, N_vec1(N_acc(ii):No_N(ii)), 1./Lambda{ii}(N_acc(ii):No_N(ii)),'-rx','MarkerIndices',(mk_ind),'Markersize',10);
end

[~,mk_ind] = intersect(N_vec1(1:N_acc(end)),equspr);
mk_ind = [mk_ind', N_acc(end)];
plot(h_a, N_vec1((1:N_acc(end))),1./Lambda{end}(1:N_acc(end)),'-b.','MarkerIndices',mk_ind,'Markersize',15);

h_l = plot(h_a, N_vec1 ,1./Lambda_lin,'--','Color',[1,1,1]*0.1);
